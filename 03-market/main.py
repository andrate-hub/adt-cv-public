...
def load_data(data_path: str, city: str, shop: str, day: str = "1-Mon") -> \
    ...
    path = os.path.join(data_path, city, shop, shop  + ".txt")
    with open(path, "r", encoding="utf-8") as file:
        ...
        except Exception as e:                             #šlo by místo Exception použít ValueError
...
def get_passed_set(data: dict[str, list[Record]], key_words: list[str]) -> set[int]:
    ...
    for key, value in data.items():         #uloží mi to key klíč a do value záznam
...
def get_q_size(data: dict[str, list[Record]], seconds: int) -> int:
    ...
    paid = get_passed_set(filtered_data, ["cash"])
...
def histogram(data: dict[str, list[Record]]) -> None:
    ...
    for i in range(8,20):
        print(f"{i}:00 --- {get_q_size(data, 15*60*60)}")

    """
     for h in range(24):
        sec = h * 3600
        q = get_q_size(data, sec)
    """
