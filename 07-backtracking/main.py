...
class SudokuSolver:
    ...
    def check_sequence(self, sequence: np.ndarray) -> bool:
        ...
        #sequence !=0 - to vnitrni vyhodnoti zda se to rovna nule a vrati to true or false, a pote se vrati hodnoty, kde je true
    ...
    def check_block(self, row_index: int, column_index: int) -> bool:
        ...
        return self.check_sequence(sequence=self.field[row_start_index:row_start_index+3, column_start_index:column_start_index+3].reshape(-1))


    def check_one_cell(self, row_index: int , column_index: int) -> bool:
        return self.check_row(row_index) and self.check_column(column_index) and self.check_block()
